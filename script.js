...
async function sendPayment(toAddress, amount) {
    ...
    try {
        const amountInWei = "0x" + (amount * 1e18).toString(16); //BigInt(Math.floor(amount * 1e18)).toString(16);
...
async function checkout() {
    ...
    try {
        ...
        for (let item of cart) {
            ...
            // Save under buyer -> artBought
            await setDoc(doc(db, "users", walletAddress, "artBought"), {
                artwork: {
                    id: item.id,
                    title: item.title,
                    imageUrl: item.imageUrl
            ...
            // Save under seller -> artSold
            await setDoc(doc(db, "users", item.sellerId, "artSold"), {
                artwork: {
                    id: item.id,
                    title: item.title,
                    imageUrl: item.imageUrl
