...
async function loadUserProfileFromDB(walletAddr) {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      if (!currentUser.username) {
        currentUser.username = "New User";
        currentUser.lastUsernameEdit = 0;
      }

      if (!currentUser.bio) {
        currentUser.bio = "No bio yet.";
        currentUser.lastBioEdit = 0;
      }

      // Merge missing fields silently
      await setDoc(userRef, {
        username: currentUser.username,
        lastUsernameEdit: currentUser.lastUsernameEdit,
        bio: currentUser.bio,
        lastBioEdit: currentUser.lastBioEdit,
        walletAddress: walletAddr,
      }, { merge: true });
    ...
    // Render the loaded/updated profile
    renderUserProfile();
...
async function saveUsername() {
  ...
  if (!newUsername) return showToast("Please enter a username.", "warning");

  const now = Date.now();
  const lastEdit = currentUser?.lastUsernameEdit || 0;
  const sixtyDays = 60 * 24 * 60 * 60 * 1000;

  if (now - lastEdit < sixtyDays) {
    const remaining = Math.ceil((sixtyDays - (now - lastEdit)) / (24 * 60 * 60 * 1000));
    return showToast(`⏳ You can change your username again in ${remaining} day(s).`, "warning");
  }
  ...
  try {
    ...
    currentUser.lastUsernameEdit = now;
...
async function saveBio() {
  ...
  if (!newBio) return showToast("Please enter a bio.", "warning");
  ...
  if (now - lastEdit < sixtyDays) {
    const remaining = Math.ceil((sixtyDays - (now - lastEdit)) / (24 * 60 * 60 * 1000));
    return showToast(`⏳ You can edit your bio again in ${remaining} day(s).`, "warning");
  }
