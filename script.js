...
function viewArtworkDetails(artworkId, source) {
    let artwork;

    if (source === 'submissions') {
        artwork = userSubmittedArtworks.find(a => a.id == artworkId);
    } else if (source === 'purchases') {
        artwork = userPurchasedArtworks.find(a => a.id == artworkId);
    } else {
        artwork = allArtworks.find(a => a.id == artworkId);
    }
    ...
    document.getElementById('modalArtworkArtist').textContent = `by ${artwork.artist}`;
    document.getElementById('modalArtworkDescription').textContent = artwork.description;
    document.getElementById('modalArtworkYear').textContent = `Year: ${artwork.year}`;
    document.getElementById('modalArtworkCategory').textContent = `Category: ${artwork.category}`;
    document.getElementById('modalArtworkDimensions').textContent = `Dimensions: ${artwork.dimensions}`;
    document.getElementById('modalArtworkPrice').textContent = `Price: ${artwork.price} ETH`;

    document.getElementById('artworkDetailsModal').style.display = 'block';
}
