...
async function checkout() {
    ...
    try {
        ...
        for (let item of cart) {
            ...
            // Save under buyer -> artBought
            // await setDoc(doc(db, "users", walletAddress.toLowerCase(), "artBought", String(item.id)), {
            //     artwork: {
            //         id: item.id,
            //         title: item.title,
            //         imageUrl: item.imageUrl
            ...
            // // Save under seller -> artSold
            // await setDoc(doc(db, "users", item.sellerId.toLowerCase(), "artSold", String(item.id)), {
            //     artwork: {
            //         id: item.id,
            //         title: item.title,
            //         imageUrl: item.imageUrl
...
//                         <p>${art.price || "0.000"} ETH</p>
//                         <button onclick="openResellModal('${docSnap.id}', '${art.title || "Untitled"}')">
//                             Resell
//                         </button>
//                     </div>
//                 </div>
        ...
        qSnap.forEach(docSnap => {
            ...
            container.innerHTML += `
                <div class="art-card">
                    <img src="${art.imageUrl}" alt="${art.title}" class="art-image">
                    <div class="art-info">
                        <h3 class="art-title">${art.title || "Untitled"}</h3>
                        <p class="art-artist">by ${art.artist || "Unknown Artist"}</p>
                        <p class="art-price">${art.price || "0.000"} ETH</p>
                        <div class="art-actions">
                            <button class="btn-primary" onclick="openResellModal('${docSnap.id}', '${art.title || "Untitled"}', ${art.price || 0})">
                                Resell
                            </button>
                        </div>
                    </div>
...
function openResellModal(artId, artTitle, currentPrice = 0) {
    ...
    document.getElementById("resellPrice").value = currentPrice || "";
...
// Open modal
function openResellModal(artId, artTitle) {
    selectedResellArtId = artId;
    document.getElementById("resellArtTitle").textContent = `Reselling: ${artTitle}`;
...
async function resellArtwork(artId, newPrice) {
    try {
        ...
        // Update UI instantly
        loadPurchasedArtworks(); // refresh purchased list
        loadSellingArtworks();   // refresh selling list
