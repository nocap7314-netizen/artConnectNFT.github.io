...
async function checkout() {
    ...
    const platformWallet = '0x742d35Cc6686C59fCC3e544961fcdeEeC4d91dc3'; // default platform wallet
...
async function validateImageUrl(url) {
    return new Promise(resolve => {
        ...
        img.onerror = () => resolve(false); // invalid
        img.src = url;
    });
}
...
async function submitArtwork(event) {
    ...
    if (!isValid) {
        showToast('Invalid image link. Please paste a direct/public link (ends in .jpg, .png, etc.)', 'error');
        return;
    }   
    ...
    try {
        ...
        if (!isValidImage) {
            showToast('Invalid image link. Please use a direct/public link (e.g. ending with .jpg or .png)', 'error');
            return;
        }
