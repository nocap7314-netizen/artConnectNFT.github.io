...
async function loadUserPurchases(walletAddr) {
    ...
    try {
        ...
        qSnap.forEach(docSnap => {
            ...
            art.category = art.category || "Uncategorized";
            art.price = art.price || "0.000";
            container.innerHTML += renderArtworkCard(art, true);
        });
    } catch (err) {
...
function renderArtworkCard(art) {
    ...
    <div class="artwork-card">
        ...
        <div class="artwork-info">
            ...
            ${isPurchased ? `
                <button onclick="openResellModal('${art.id}', '${art.title}')">Resell</button>
            ` : ""}
        </div>
    </div>`;
}
