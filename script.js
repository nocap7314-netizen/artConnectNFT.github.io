...
async function loadArtists() {
  ...
  try {
    ...
    //   </div>
    artistsGrid.innerHTML = artists.map(artist => `
      <div class="artist-card" onclick="showArtistProfile('${walletAddress}')">
        <div class="artist-header">
            <div class="artist-avatar">${username.charAt(0).toUpperCase()}</div>
            <div class="artist-info">
            <h3>${username}</h3>
            <p>${bio ? bio.slice(0, 40) + "..." : "No bio yet."}</p>
            </div>
        </div>

        <div class="artist-stats">
            <span><i class="fa-solid fa-image"></i> ${artCount} Artworks</span>
            <span><i class="fa-brands fa-ethereum"></i> ${totalSales} tETH</span>
        </div>

        <div class="artist-stats">
            <span><i class="fa-regular fa-calendar"></i> ${joinedYear}</span>
        </div>
    </div>
