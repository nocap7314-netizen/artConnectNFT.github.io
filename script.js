...
async function loadArtworks() {
    try {
        ...
        // Always attach Firestore doc ID
        const artworks = querySnapshot.docs.map(docSnap => ({
            id: docSnap.id,
            ...docSnap.data()
        }));
        ...
        if (artworks.length === 0) {
            gallery.innerHTML = `<p>No artworks available at the moment.</p>`;
            return;
        }
        ...
        artworks.forEach(item => {
            ...
            card.innerHTML = `
                <img src="${item.imageUrl}" alt="${item.title}" class="art-image">
                <h3 class="art-title">${item.title}</h3>
                <p class="art-artist">by ${item.artist}</p>
                <p class="art-price">${item.price} ETH</p>
        ...
        gallery.querySelectorAll('.btn-add').forEach(btn => {
        ...
        gallery.querySelectorAll('.btn-view').forEach(btn => {
            btn.addEventListener('click', e => {
                const artId = e.target.getAttribute('data-id');
                viewDetails(artId);
            });
...
async function checkout() {
    ...
    try {
        ...
        for (let item of cart) {
            ...
            // Pay seller
            const txSeller = await sendPayment(item.sellerId, sellerAmount);
            await waitForTransactionReceipt(txSellerHash);
